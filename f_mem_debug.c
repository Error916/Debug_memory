...
typedef struct{
	...
	unsigned int alloc_alocated;
	unsigned int size;
	unsigned int alocated;
	unsigned int freed;
}STMemAllocLine;
...
void f_debug_mem_add(void *pointer, unsigned int size, char *file, unsigned int line){
	unsigned int i, j;
...
unsigned char f_debug_mem_remove(void *buf){
	unsigned int i, j, k;
	for(i = 0; i < f_alloc_line_count; i++){
		for(j = 0; j < f_alloc_lines[i].alloc_count; j++){
...
void *f_debug_mem_realloc(void *pointer, unsigned int size, char *file, unsigned int line){
...
signed int f_debug_mem_consumption(){
